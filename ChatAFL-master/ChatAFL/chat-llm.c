...
// Function to initialize the log file if not already opened
void initialize_log_file(const char *filename)
{
    if (!log_file)
    {
...
// Function to log the prompt and model as plain text
void log_prompt(const char *prompt, const char *model)
{
...
// Log the prompt and model before sending it
    log_prompt(prompt, "Prompt");    

    CURL *curl;
    CURLcode res = CURLE_OK;
    char *answer = NULL;
    char *url = "http://llm-server:8000/chat-llm";
    
    char *content_header = "Content-Type: application/json";
    char *accept_header = "Accept: application/json";
    char *data = NULL;
    
    // Construct the JSON request payload matching FuzzerRequestModel
    asprintf(&data, 
        "{"
        "\"question\": \"%s\" "
        "}",
    ...
    do
    {
        ...
        if (curl)
        {
            ...
            if (res == CURLE_OK)
            {
                ...
                if (json_object_object_get_ex(jobj, "response", &resp_obj))
                {
                    const char *data = json_object_get_string(resp_obj);
                    if (data)
                    {
                        answer = strdup(data);
                    }
...
char *construct_prompt_stall(char *protocol_name, char *examples, char *history)
{
    ...
    char *template = "In the %s protocol, the communication history between the client and the server is as follows:"
                     "Communication History:\\n\\\"\\\"\\\"\\n%s\\\"\\\"\\\"\n"
                     "Based on your observations, and according to the protocol state machine, Obtain the necessary information about the state machine/model,"
                     " and provide the next proper client request (packet) that can affect the server's state, triggering a new state transition.";

    char *prompt = NULL;
    // asprintf(&prompt, template, protocol_name, protocol_name, protocol_name, examples, history);
    asprintf(&prompt, template, protocol_name, history);
...
char *construct_prompt_for_templates(char *protocol_name, char **final_msg) {
    ...
    asprintf(&msg, grammar_prompt, protocol_name, protocol_name);
    
    *final_msg = msg;
    
    // Return a copy of the original grammar_prompt format string
    char *prompt_grammars = strdup(grammar_prompt);
...
char *construct_prompt_for_remaining_templates(char *protocol_name, char *first_question, char *first_answer) {
    ...
    asprintf(&second_question, 
        "For the %s protocol, you are asked to provide the remaining client request/method templates, "
        "including optional headers. Obtain necessary information about the remaining client requests/methods "
        "used in a client-server interaction, and also obtain information about their header fields.", 
        protocol_name);

    // Build the complete prompt in the exact requested format
    asprintf(&prompt,
        "Previous Question:\\n%s\\n\\n"
        "It's answer:\\n%s\\n\\n"
        "Current question for you to answer now:\\n%s",
        first_question,
        first_answer,
        second_question);
...
char *enrich_sequence(char *sequence, khash_t(strSet) * missing_message_types)
{
    const char *prompt_template =
        ...
        "%.*s\\n"
        "Obtain necessary information about %.*s client requests and the client state model/machine,"
